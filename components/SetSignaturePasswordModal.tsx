...
interface SetSignaturePasswordModalProps {
  onClose: () => void;
  onSetPassword: (password: string) => boolean;
}

const SetSignaturePasswordModal: React.FC<SetSignaturePasswordModalProps> = ({ onClose, onSetPassword }) => {
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col animate-slide-up" onClick={e => e.stopPropagation()}>
        <header className="p-6 border-b border-slate-200 dark:border-slate-700">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Senha de Assinatura</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Esta senha será usada para confirmar suas assinaturas nos relatórios.
          </p>
        </header>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block mb-2 text-sm font-medium text-slate-900 dark:text-white">Nova Senha de Assinatura</label>
            <input 
              type="password"
              value={newPassword}
...
export default SetSignaturePasswordModal;